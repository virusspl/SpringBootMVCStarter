...
public class DocViewCacheScheduler {
	...
	public void cacheDocViewFolders() {
		...
		// clear old cache
		net.sf.ehcache.CacheManager.getInstance().clearAllStartingWith("pdfListFromDirectory");
		// start time
		long start = System.currentTimeMillis();
		// loop cache list
		for(String code: docViewController.getCacheList()){
			fileHelper.getPdfMap(new File(env.getRequiredProperty(code)));
		}
		// prepare result text
		String result = "Docview directory cache (time: " + ((System.currentTimeMillis() - start) / 1000) + "s)";
		// save parameter
		parametersService.storeSystemInfoParameter("SCHDL-DOCVIEW-ATW", "Directory cache", result);
		// log job
		logger.info(result);
	}
