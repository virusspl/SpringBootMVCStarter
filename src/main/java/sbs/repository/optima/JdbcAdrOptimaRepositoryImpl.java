...
public class JdbcAdrOptimaRepositoryImpl implements JdbcAdrOptimaRepository {
	...
	public HrUserInfo findCurrentlyEmployedById(String userId) {
		List<Map<String, Object>> resultSet = jdbc.queryForList(
				...
				+ "[PRE_ZatrudnionyDo] "
				+ "FROM "
				+ "[CDN_ADR_S_A_2017].[CDN].[Dzialy] "
				+ "INNER JOIN (([CDN_ADR_S_A_2017].[CDN].[PracEtaty] "
				+ "INNER JOIN [CDN_ADR_S_A_2017].[CDN].[DaneKadMod] "
				+ "ON [PRE_ETADkmIdStanowisko] = [DKM_DkmId]) "
				+ "INNER JOIN [CDN_ADR_S_A_2017].[CDN].[PracKartyRcp] "
